...
import { type CreateOrUpdateUserRequestData, type GetUserData, type DeleteUserData } from "@/api/system/user/types/user"
...
//# 强制下线
const handleOffine = (row: GetUserData) => {
  ElMessageBox.confirm(`正在强制下线用户：${row.username}，确认强制下线？`, "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(() => {
    websocketStore
      .sendOffineMessage(row.userid)
      .then(() => {
        // 成功发送后可以添加成功的提示（可选）
        ElMessage({
          message: "强制下线命令已发送",
      ...
      .catch((error) => {
        // 发送失败时显示错误信息
        ElMessage({
          message: `发送强制下线命令失败: ${error.message}`,
          type: "error"
        })
      })
  })
  ...
  <div class="app-container">
    ...
    <el-card v-loading="loading" shadow="never">
      ...
      <div class="table-wrapper">
        <el-table :data="userData" show-overflow-tooltip @selection-change="handleSelectionChange">
          ...
          <el-table-column fixed="right" label="操作" width="350" align="center">
            <template #default="scope">
              <el-button type="primary" text icon="Edit" bg size="small" @click="handleUpdate(scope.row)"
                >编辑</el-button
              >
              <el-button type="info" text icon="SwitchButton" bg size="small" @click="handleOffine(scope.row)"
                >下线</el-button
              >
